...
# КУРС MORTISCOIN
# ───────────────────────────────────────────────
MORTIS_COIN_RATE = 5.0
MORTIS_COIN_LAST_CHANGED = None

def format_number(num: int) -> str:
    return f"{num:,}".replace(",", " ")

def get_may_9_discount(price: int, member: discord.Member = None) -> tuple:
    """Расчет скидки на 9 мая (День Победы)
    Возвращает (финальная_цена, размер_скидки, активна_ли_скидка)
    """
    try:
        import pytz
        tz = pytz.timezone(MAY_9_DISCOUNT_TIMEZONE)
        today = datetime.now(tz).date()
        
        # Проверяем, 9 мая ли
        is_may_9 = (today.month == 5 and today.day == 9)
        
        if not is_may_9:
            return price, 0, False
        
        # Проверяем роль если передан member
        if member:
            has_role = any(role.name in MAY_9_DISCOUNT_ROLE_NAMES for role in member.roles)
            if not has_role:
                return price, 0, False
        
        discount_amount = int(price * (MAY_9_DISCOUNT_PERCENT / 100))
        final_price = price - discount_amount
        return final_price, discount_amount, True
    except Exception as e:
        print(f"⚠️  Ошибка при расчете скидки на 9 мая: {e}")
        return price, 0, False
...
MAY_9_DISCOUNT_ROLE_NAMES = ["Мужской"]  # Роли для скидки
MAY_9_DISCOUNT_TIMEZONE = "Europe/Moscow"  # МСК = UTC+3
...
# ───────────────────────────────────────────────
def get_user_discount(user_id: str, member: discord.Member = None) -> int:
    """Возвращает процент скидки пользователя на основе активных скидочных карт и акций"""
    if user_id not in economy_data:
        return 0
    
    # Проверяем скидку на 9 мая
    if member:
        _, _, has_may_discount = get_may_9_discount(100, member)
        if has_may_discount:
...
class ShopCategorySelect(Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        discount_percent = get_user_discount(user_id, interaction.user)
...
async def userinfo(ctx: commands.Context, member: discord.Member = None):
    try:
        ...
        discount_percent = get_user_discount(user_id, member) if has_full_access(ctx.guild.id) else 0
...
async def balance(ctx: commands.Context, member: discord.Member = None):
    try:
        ...
        discount_percent = get_user_discount(user_id, member)
